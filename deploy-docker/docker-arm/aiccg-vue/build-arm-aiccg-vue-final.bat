@echo off
chcp 65001 > nul
:: 最终版 Docker ARM64 AICCG Vue前端镜像构建脚本

echo ========================================
echo   最终版 Docker ARM64 AICCG Vue前端镜像构建脚本
echo ========================================
...
if %errorlevel% equ 0 (
    echo [✓] nginx:alpine 镜像拉取成功
) else (
    echo [!] nginx:alpine 镜像拉取失败，将继续使用本地镜像
)

:: 构建ARM64架构的AICCG Vue前端镜像
echo [5/6] 构建AICCG Vue前端 ARM64镜像...
cd ..\..\..\jeecgboot-vue3

:: 使用buildx构建ARM64镜像
docker buildx build --platform linux/arm64 -t aiccg-vue3-arm64-final --load .
...
docker images | findstr aiccg-vue3-arm64-final
if %errorlevel% equ 0 (
    echo [✓] 镜像验证成功
) else (
    echo [✗] 镜像验证失败
...
:: 检查镜像架构
echo [7/7] 检查镜像架构...
docker inspect aiccg-vue3-arm64-final | findstr -i "Architecture"
if %errorlevel% equ 0 (
    echo [✓] 架构检查完成
) else (
    echo [✗] 架构检查失败
...
echo ========================================
echo 镜像名称: aiccg-vue3-arm64-final
