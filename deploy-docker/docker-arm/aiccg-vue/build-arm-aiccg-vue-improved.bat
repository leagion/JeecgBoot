@echo off
chcp 65001 > nul
:: 改进版 Docker ARM64 AICCG Vue前端镜像构建脚本

echo ========================================
echo   改进版 Docker ARM64 AICCG Vue前端镜像构建脚本
echo ========================================
...
echo [3/4] 构建AICCG Vue前端 ARM64镜像...
cd ..\..\..\jeecgboot-vue3
:: 使用国内镜像源构建ARM64架构镜像
docker buildx build --platform linux/arm64 -t aiccg-vue3-arm64 --load .
...
docker images | findstr aiccg-vue3-arm64
if %errorlevel% equ 0 (
    echo [✓] 镜像验证成功
) else (
    echo [✗] 镜像验证失败
...
:: 检查镜像架构
echo [5/5] 检查镜像架构...
docker inspect aiccg-vue3-arm64 | findstr -i "Architecture"
if %errorlevel% equ 0 (
    echo [✓] 架构检查完成
) else (
    echo [✗] 架构检查失败
