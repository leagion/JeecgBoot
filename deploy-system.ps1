...
Write-Host "========================================"
Write-Host "  AICCG 系统全自动部署脚本"
Write-Host "========================================"
Write-Host ""

if (-not $SkipBuild) {
    # 检查必要工具
    Write-Host "[1/4] 检查必要工具..."
    ...
    # 编译后端项目
    Write-Host "[3/4] 编译后端项目..."
    Set-Location -Path "jeecg-boot"
    $mvnResult = Start-Process -FilePath "mvn" -ArgumentList "clean", "install", "-Pdocker" -NoNewWindow -Wait -PassThru
    ...
    # 编译前端项目
    Write-Host "[4/4] 编译前端项目..."
    Set-Location -Path "jeecgboot-vue3"
    $pnpmInstallResult = Start-Process -FilePath "pnpm" -ArgumentList "install" -NoNewWindow -Wait -PassThru
    if ($pnpmInstallResult.ExitCode -ne 0) {
        Write-Host "[错误] 前端依赖安装失败！" -ForegroundColor Red
        Set-Location -Path ".."
        exit 1
    }

    $pnpmBuildResult = Start-Process -FilePath "pnpm" -ArgumentList "run", "build:docker" -NoNewWindow -Wait -PassThru
    if ($pnpmBuildResult.ExitCode -ne 0) {
    ...
    Write-Host "跳过编译步骤..." -ForegroundColor Yellow
}

# 启动Docker容器
Write-Host "正在启动Docker容器..."
