...
<template>
  <PageLayout navTitle="lq_leadersay"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqLeadersayData';
defineOptions({
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqLeadersay/lqLeadersay/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqLeadersay/lqLeadersay/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
