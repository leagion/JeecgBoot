...
navigationBarTitleText: 'lq_leadersay',
},
}
</route>
<template>
        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  ...
                  type="date"
                  name='sayDate'
                  v-model:value="myFormData['sayDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['leadername']"
                  :label="get4Label('首长姓名')"
                  name='leadername'
                  prop='leadername'
                  placeholder="请选择首长姓名"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
                :label="get4Label('首长指示')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['leadersay']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('落实情况')"
                 labelWidth="100px"
                 type="textarea"
                 name='doit'
                 prop='doit'
                 clearable
                 :maxlength="300"
                v-model="myFormData['doit']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['responsibleunit']"
                  :label="get4Label('责任单位')"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('完成时限')"
                  labelWidth="100px"
                  type="date"
                  name='validityperiod'
                  v-model:value="myFormData['validityperiod']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['isfinished']"
                  :label="get4Label('是否完成')"
                  name='isfinished'
                  prop='isfinished'
                  placeholder="请选择是否完成"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['isshow']"
                  :label="get4Label('是否显示')"
                  name='isshow'
                  prop='isshow'
                  placeholder="请选择是否显示"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('LqLeadersayList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lqLeadersay/lqLeadersay/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lqLeadersay/lqLeadersay/edit':'/lqLeadersay/lqLeadersay/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
