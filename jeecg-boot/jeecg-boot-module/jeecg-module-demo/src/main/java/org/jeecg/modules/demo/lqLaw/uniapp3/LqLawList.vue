...
layout: 'default',
style: {
navigationBarTitleText: '综合执法',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="综合执法"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqLaw/lqLaw/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqLaw/lqLaw/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
