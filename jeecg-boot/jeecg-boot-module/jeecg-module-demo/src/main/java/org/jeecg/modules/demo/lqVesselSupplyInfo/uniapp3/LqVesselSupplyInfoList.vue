...
layout: 'default',
style: {
navigationBarTitleText: '后装保障',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="后装保障"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqVesselSupplyInfoData';
defineOptions({
  name: 'LqVesselSupplyInfoList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqVesselSupplyInfo/lqVesselSupplyInfo/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqVesselSupplyInfo/lqVesselSupplyInfo/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
