...
const dataId = ref('')
const backRouteName = ref('LqMindmapTagList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lqMindmapTag/lqMindmapTag/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lqMindmapTag/lqMindmapTag/edit':'/lqMindmapTag/lqMindmapTag/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
