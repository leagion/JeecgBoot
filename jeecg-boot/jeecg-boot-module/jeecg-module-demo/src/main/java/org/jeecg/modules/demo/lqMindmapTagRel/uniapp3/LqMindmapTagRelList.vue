...
layout: 'default',
style: {
navigationBarTitleText: 'lq_mindmap_tag_rel',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="lq_mindmap_tag_rel"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqMindmapTagRelData';
defineOptions({
  name: 'LqMindmapTagRelList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqMindmapTagRel/lqMindmapTagRel/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqMindmapTagRel/lqMindmapTagRel/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
