...
<template>
  <PageLayout navTitle="舰机统计"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqAirShipCountData';
defineOptions({
  name: 'LqAirShipCountList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqAirShipCount/lqAirShipCount/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqAirShipCount/lqAirShipCount/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
