...
layout: 'default',
style: {
navigationBarTitleText: 'lq_mindmap_version',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="lq_mindmap_version"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqMindmapVersionData';
defineOptions({
  name: 'LqMindmapVersionList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqMindmapVersion/lqMindmapVersion/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqMindmapVersion/lqMindmapVersion/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
