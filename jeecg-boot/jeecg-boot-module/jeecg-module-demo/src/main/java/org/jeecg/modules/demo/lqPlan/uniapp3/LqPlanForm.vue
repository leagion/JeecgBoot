        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  ...
                  type="date"
                  name='planstartdate'
                  v-model:value="myFormData['planstartdate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('计划结束日期')"
                  labelWidth="100px"
                  type="date"
                  name='planenddate'
                  v-model:value="myFormData['planenddate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['missionplace']"
                  :label="get4Label('任务点位')"
                  name='missionplace'
                  prop='missionplace'
                  placeholder="请选择任务点位"
                  :rules="[
                                  { required: true, message: '请输入任务点位!'},
                  ]"
                  clearable
              />
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['missionperson']"
                  :label="get4Label('力力')"
                  name='missionperson'
                  prop='missionperson'
                  placeholder="请选择力力"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['missionvessel']"
                  :label="get4Label('号号')"
                  name='missionvessel'
                  prop='missionvessel'
                  placeholder="请选择号号"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
                :label="get4Label('任务内容')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['missioncontent']"
              ></wd-textarea>
        </view>
          </wd-cell-group>
        </wd-form>
...
const dataId = ref('')
const backRouteName = ref('LqPlanList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lqPlan/lqPlan/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/lqPlan/lqPlan/edit':'/lqPlan/lqPlan/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
