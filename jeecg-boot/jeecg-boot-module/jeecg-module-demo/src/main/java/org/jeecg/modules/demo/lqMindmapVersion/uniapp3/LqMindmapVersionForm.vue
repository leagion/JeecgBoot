        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('关联思维导图ID')"
                  name='mindmapId'
                  prop='mindmapId'
                  placeholder="请选择关联思维导图ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['contentDiff']"
                  :label="get4Label('与上一版本的差异内容（加密存储）')"
                  name='contentDiff'
                  prop='contentDiff'
                  placeholder="请选择与上一版本的差异内容（加密存储）"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['isKeyVersion']"
                  :label="get4Label('是否为关键版本')"
                  name='isKeyVersion'
                  prop='isKeyVersion'
                  placeholder="请选择是否为关键版本"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('操作人ID')"
                  name='userId'
                  prop='userId'
                  placeholder="请选择操作人ID"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('LqMindmapVersionList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lqMindmapVersion/lqMindmapVersion/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lqMindmapVersion/lqMindmapVersion/edit':'/lqMindmapVersion/lqMindmapVersion/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
