      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['topicText']"
                  :label="get4Label('主题文本')"
                  name='topicText'
                  prop='topicText'
                  placeholder="请选择主题文本"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('AI生成结果')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['aiResult']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['hitCount']"
                   :label="get4Label('命中次数')"
                   name='hitCount'
                   prop='hitCount'
                   placeholder="请选择命中次数"
                   inputMode="numeric"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('用户ID，为空表示公共缓存')"
                  name='userId'
                  prop='userId'
                  placeholder="请选择用户ID，为空表示公共缓存"
                  :rules="[
                  ]"
                  clearable
              />
...
const dataId = ref('')
const backRouteName = ref('LqMindmapAiCacheList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lqMindmapAiCache/lqMindmapAiCache/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lqMindmapAiCache/lqMindmapAiCache/edit':'/lqMindmapAiCache/lqMindmapAiCache/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
