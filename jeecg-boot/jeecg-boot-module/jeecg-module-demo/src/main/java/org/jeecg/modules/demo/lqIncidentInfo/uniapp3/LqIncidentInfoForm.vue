    ...
    <scroll-view class="scrollArea" scroll-y>
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('事发地')"
                 ...
                 dict="placeName"
                v-model="myFormData['incidentLocation']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-select
                :label="get4Label('船籍国')"
                 labelWidth="100px"
                 type="list"
                 name='shipRegistrationCountry'
                 dict="countryName"
                v-model="myFormData['shipRegistrationCountry']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['shipNumber']"
                  :label="get4Label('船舷号')"
                  name='shipNumber'
                  prop='shipNumber'
                  placeholder="请选择船舷号"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('事件经过')"
                 labelWidth="100px"
                 type="textarea"
                 name='incidentProcess'
                 prop='incidentProcess'
                 clearable
                 :maxlength="300"
                v-model="myFormData['incidentProcess']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['ourShipNumber']"
                  :label="get4Label('我方舷号')"
                  name='ourShipNumber'
                  prop='ourShipNumber'
                  placeholder="请选择我方舷号"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('LqIncidentInfoList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lqIncidentInfo/lqIncidentInfo/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/lqIncidentInfo/lqIncidentInfo/edit':'/lqIncidentInfo/lqIncidentInfo/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
