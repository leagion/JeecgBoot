...
layout: 'default',
style: {
navigationBarTitleText: '气象海况',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="气象海况"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import { http } from '../../../../../../../../../../../../jeecgboot-vue3/src/utils/http'
import usePageList from '@/hooks/usePageList'
import {columns} from './LqWeatherseaData';
defineOptions({
  name: 'LqWeatherseaList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqWeathersea/lqWeathersea/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqWeathersea/lqWeathersea/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
