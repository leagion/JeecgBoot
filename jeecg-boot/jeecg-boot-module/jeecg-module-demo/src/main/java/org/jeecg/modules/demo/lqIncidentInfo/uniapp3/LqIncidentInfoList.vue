...
<template>
  <PageLayout navTitle="专项应对"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqIncidentInfoData';
defineOptions({
  name: 'LqIncidentInfoList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqIncidentInfo/lqIncidentInfo/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqIncidentInfo/lqIncidentInfo/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
