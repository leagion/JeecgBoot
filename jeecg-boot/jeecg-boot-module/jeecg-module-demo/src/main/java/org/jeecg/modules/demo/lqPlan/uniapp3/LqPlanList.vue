...
<template>
  <PageLayout navTitle="重要计划"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqPlanData';
defineOptions({
  name: 'LqPlanList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqPlan/lqPlan/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqPlan/lqPlan/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
