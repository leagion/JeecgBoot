...
layout: 'default',
style: {
navigationBarTitleText: '值班人员',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="值班人员"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqDutyData';
defineOptions({
  name: 'LqDutyList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqDuty/lqDuty/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqDuty/lqDuty/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
