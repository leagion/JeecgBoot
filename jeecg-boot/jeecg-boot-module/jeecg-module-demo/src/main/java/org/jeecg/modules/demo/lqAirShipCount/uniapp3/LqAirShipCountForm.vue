        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  ...
                  type="date"
                  name='incidentDate'
                  v-model:value="myFormData['incidentDate']"
              ></online-date>
        </view>
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('国籍')"
                 ...
                 name='nationality'
                 dict="countryName"
                v-model="myFormData['nationality']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['shipAircraftNumber']"
                  :label="get4Label('舰机号')"
                  name='shipAircraftNumber'
                  prop='shipAircraftNumber'
                  placeholder="请选择舰机号"
                  :rules="[
                  ]"
                  clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['activityArea']"
                  :label="get4Label('活动区域')"
                  name='activityArea'
                  prop='activityArea'
                  placeholder="请选择活动区域"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
                :label="get4Label('侵权情况')"
                 labelWidth="100px"
                 type="textarea"
                 name='infringementSituation'
                 prop='infringementSituation'
                 clearable
                 :maxlength="300"
                v-model="myFormData['infringementSituation']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('我应对情况')"
                 labelWidth="100px"
                 type="textarea"
                 name='ourResponse'
                 prop='ourResponse'
                 clearable
                 :maxlength="300"
                v-model="myFormData['ourResponse']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
...
const dataId = ref('')
const backRouteName = ref('LqAirShipCountList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lqAirShipCount/lqAirShipCount/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/lqAirShipCount/lqAirShipCount/edit':'/lqAirShipCount/lqAirShipCount/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
