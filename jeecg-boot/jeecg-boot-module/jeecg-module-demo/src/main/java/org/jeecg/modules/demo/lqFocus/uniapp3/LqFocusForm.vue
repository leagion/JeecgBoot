          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
                :label="get4Label('关注内容')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['focuscontent']"
              ></wd-textarea>
        </view>
          </wd-cell-group>
        </wd-form>
...
const dataId = ref('')
const backRouteName = ref('LqFocusList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lqFocus/lqFocus/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/lqFocus/lqFocus/edit':'/lqFocus/lqFocus/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
