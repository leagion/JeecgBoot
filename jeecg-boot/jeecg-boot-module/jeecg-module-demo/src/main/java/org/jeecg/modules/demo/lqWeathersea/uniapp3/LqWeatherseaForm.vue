      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['weather']"
                  :label="get4Label('天气(晴、雨）')"
                  name='weather'
                  prop='weather'
                  placeholder="请选择天气(晴、雨）"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['windDirection']"
                  :label="get4Label('风向')"
                  name='windDirection'
                  prop='windDirection'
                  placeholder="请选择风向"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['windScale']"
                   :label="get4Label('风级(几级)')"
                   name='windScale'
                   prop='windScale'
                   placeholder="请选择风级(几级)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['waveHeight']"
                   :label="get4Label('浪高(米)')"
                   name='waveHeight'
                   prop='waveHeight'
                   placeholder="请选择浪高(米)"
                   inputMode="numeric"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['waveDirection']"
                  :label="get4Label('浪向')"
                  name='waveDirection'
                  prop='waveDirection'
                  placeholder="请选择浪向"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['seaCondition']"
                  :label="get4Label('海况(几级)')"
                  name='seaCondition'
                  prop='seaCondition'
                  placeholder="请选择海况(几级)"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['visibility']"
                   :label="get4Label('能见度(海里)')"
                   name='visibility'
                   prop='visibility'
                   placeholder="请选择能见度(海里)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
...
const dataId = ref('')
const backRouteName = ref('LqWeatherseaList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lqWeathersea/lqWeathersea/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/lqWeathersea/lqWeathersea/edit':'/lqWeathersea/lqWeathersea/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
