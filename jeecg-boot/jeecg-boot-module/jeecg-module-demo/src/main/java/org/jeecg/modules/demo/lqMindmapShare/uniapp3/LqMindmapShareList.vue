...
<template>
  <PageLayout navTitle="lq_mindmap_share"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqMindmapShareData';
defineOptions({
  name: 'LqMindmapShareList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqMindmapShare/lqMindmapShare/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqMindmapShare/lqMindmapShare/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
