      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['tagId']"
                  :label="get4Label('标签ID')"
                  name='tagId'
                  prop='tagId'
                  placeholder="请选择标签ID"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('LqMindmapTagRelList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lqMindmapTagRel/lqMindmapTagRel/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lqMindmapTagRel/lqMindmapTagRel/edit':'/lqMindmapTagRel/lqMindmapTagRel/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
