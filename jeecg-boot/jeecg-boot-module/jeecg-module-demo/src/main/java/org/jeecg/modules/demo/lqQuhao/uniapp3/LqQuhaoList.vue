...
<template>
  <PageLayout navTitle="文件取号"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqQuhaoData';
defineOptions({
  name: 'LqQuhaoList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqQuhao/lqQuhao/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqQuhao/lqQuhao/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
