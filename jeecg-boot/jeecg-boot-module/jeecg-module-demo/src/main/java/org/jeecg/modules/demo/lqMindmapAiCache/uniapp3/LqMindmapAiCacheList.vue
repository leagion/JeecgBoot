...
layout: 'default',
style: {
navigationBarTitleText: 'lq_mindmap_ai_cache',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="lq_mindmap_ai_cache"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqMindmapAiCacheData';
defineOptions({
  name: 'LqMindmapAiCacheList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqMindmapAiCache/lqMindmapAiCache/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqMindmapAiCache/lqMindmapAiCache/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
