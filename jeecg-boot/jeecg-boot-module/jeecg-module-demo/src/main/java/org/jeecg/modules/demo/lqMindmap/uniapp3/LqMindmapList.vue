...
<template>
  <PageLayout navTitle="lq_mindmap"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LqMindmapData';
defineOptions({
  name: 'LqMindmapList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lqMindmap/lqMindmap/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lqMindmap/lqMindmap/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
